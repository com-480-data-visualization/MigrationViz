...
function proportionalAreaCircle() {
    const margin = {top: 20, right: 20, bottom: 400, left: 200},
    width = 960 - margin.left - margin.right,
    height = 500 - margin.top - margin.bottom;

const sumTotal = 10; //Needs to be calculated
...
data.forEach(entry => {
    ...
    const date = parseDate(entry.date_sorted)
    const month = date.getMonth() + 1; // JavaScript months are zero-based, so add 1
...
const parsedYear = parseInt(year) < 93 ? '20' + year : '19' + year;

// Construct a new date object with the components in the correct order
...
const margin = {top: 20, right: 0, bottom: 450, left: 40}, // right need to be 0, so that it is exactly on the line of Europe carte right. Left needs space, in order to see the scale. 
...
var timeline = d3.select("#my_dataviz").append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", height + margin.top + margin.bottom)
    .append("g")
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
...
const maxSum = Object.values(sumPerMonth)
    .reduce((max, yearData) => {
        const yearMax = Math.max(...Object.values(yearData));
